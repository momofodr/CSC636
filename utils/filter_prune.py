...
def get_masks(model, pruning_ratio):
    ...
    for p in model.parameters():
        if len(p.data.size()) == 4:
            all_weights += list(torch.sum(p.data.abs(), dim=(1,2,3)).cpu().numpy().flatten())
    ...
    for p in model.parameters():
        if len(p.data.size()) == 4:
            mask = (torch.sum(p.data.abs(), dim=(1,2,3)) > threshold).float()
            masks.append(mask)
...
def set_masks(model, masks):
    ...
    for p in model.parameters():
        if len(p.data.size()) == 4:
            p.data = p.data * torch.reshape(masks[i], [-1,1,1,1])
            i += 1
